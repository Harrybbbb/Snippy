import React from 'react'
import { content } from './Cardcontent'
import { Link } from 'react-router-dom'

export default function Dsa() {
  const filteredContent = content.filter ((item) => item.category === "DSA")
  return (
    <>
    <div className='type px-10 text-3xl text-white font-basic font-semibold'><p>Sorting</p></div>
    <div className="container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-10 px-4 sm:px-6 md:px-10 h-auto py-10">
      {filteredContent.map((item) => (
         <div
         key={item.id}
         className="card flex flex-col card bg-black rounded-lg p-4 sm:px-5 sm:py-5 shadow-2xl shadow-[#3b82f6]/10 gap-3"
         style={{ border: "1px solid grey" }}
       >
         <div className="date text-grey font-basic font-light">
           {item.date}
         </div>
         <div className="title text-white text-xl sm:text-2xl font-basic">
           <p>{item.title}</p>
         </div>
         <div className="language text-grey text-xs sm:text-sm font-basic">
           <p>{item.category}</p>
         </div>
         <div className="code-button flex flex-row justify-center">
           <Link to= {`/code/${item.id}`}>
             <button className="h-7 sm:h-8 text-xs sm:text-sm rounded-md px-2 font-basic text-white bg-purple border-2 border-solid border-purple  transition ease-in-out delay-100 hover:scale-110 shadow-lg shadow-purple/50">
               View Code
             </button>
           </Link>
         </div>
       </div>
      ))}
    </div>
    </>
  )
}
